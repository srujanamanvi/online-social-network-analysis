...
def bfs(graph, root, max_depth):
    ...
    while d:
        u = d.popleft()
        children = graph.neighbors(u)
        if dist[u]+1 <= max_depth:
            for v in children:
                if v not in visited:
                    visited.append(v)
                    d.append(v)
                    dist[v] = dist[u] + 1
                if v not in node2num_paths: 
                    n = [x for x in graph.neighbors(v) if x in visited and dist[x] < dist[v]]
                    node2num_paths[v] = len(n)
                    node2parents[v] = n
    return dist, node2num_paths, node2parents
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    node_credit = {}
    edge_credit = {}
    n = node2parents.items()
    for x in n:
        for u in x[1]:
            new_d[u].append(x[0])
    sorted(new_d.items())
    val = node2distances.items()
    sorted_by_second = sorted(val, key=lambda x: x[1], reverse=True)
    nodes=[x[0] for x in sorted_by_second]
    for i in nodes:
        if i not in new_d.keys():
            ...
            children = new_d[i]
            for y in children:
                value = node2num_paths[y]
                l = sorted([i,y])
                tup = tuple(l)
                edge_credit[tup] = node_credit[y]/value
                node_credit_val = node_credit_val + edge_credit[tup]
            node_credit[i] = 1 + node_credit_val
    return edge_credit 
...
def approximate_betweenness(graph, max_depth):
    ...
    for x in n:
        ...
        res = Counter(result)
        sum1 = sum1 + res
    res_dictionary = dict(sum1)
    for key, value in res_dictionary.items():
        res_dictionary[key] = value / 2
...
def get_subgraph(graph, min_degree):
    ...
    """
    deg = graph.degree()
    to_keep = [n for n in deg if deg[n] >= min_degree]
    new_graph = graph.subgraph(to_keep)
    return new_graph
...
def norm_cut(S, T, graph):
    ...
    norm_val = float((c / vol_s) + (c / vol_t))
    return norm_val
...
def make_training_graph(graph, test_node, n):
    ...
    As inlecture, we'll assume there is a test_node for which we will
    ...
    cnt = 1
    H = graph.copy()
    neighbors = H.neighbors(test_node)
    for i in sorted(neighbors):
        if cnt <= n:
            H.remove_edge(i, test_node)
            cnt = cnt+1
    return H
...
def jaccard(graph, node, k):
    ...
    s = sorted(scores.items(), key=lambda x: x[0][1])
    return sorted(s, key=lambda x: (x[1]), reverse=True)[0:k]
...
def path_score(graph, root, k, beta):
    ...
    for x in node2distances.items():
        if x[0]!=root and x[0] not in graph.neighbors(root):
            new_distances[(root, x[0])] = node2distances[x[0]]
    
    for i in node2num_paths.items():
        if i[0]!=root and i[0] not in graph.neighbors(root):
            num_paths_from_root[(root, i[0])] = node2num_paths[i[0]]
                   
    path_s = {}
    i = 0
    n = 0
    for t in new_distances.items():
        i = t[1]
        n = num_paths_from_root[t[0]]
        path_s[t[0]] = (beta**i)*n
    s = sorted(path_s.items(), key=lambda x: x[0][1])
    return sorted(s, key=lambda x: (x[1]), reverse=True)[0:k]
...
def main():
    ...
    print('Total Time took :' + str(time() - start_time) + ' Sec')
